...
def uppasd_jfile_text(vectors: np.ndarray, jij: np.ndarray, structure: CrystalStructure) -> str:
    """Return UppASD-style exchange text for single-sublattice ``J(R)``.

    The exported columns are ``iatom jatom r_x r_y r_z Jij |rij|``.  The
    lattice translations are integer direct-lattice coordinates, ``Jij`` is
    in mRy, and distances are in Angstrom.
    """
    distances = np.linalg.norm(np.asarray(vectors, dtype=int) @ structure.lattice_angstrom, axis=1)
    order = np.lexsort((vectors[:, 2], vectors[:, 1], vectors[:, 0], distances))
    lines = []
    # lines = [
    ...
    for idx in order:
        ...
        lines.append(
            ...
            f"{float(jij[idx].real):18.10f} {float(distances[idx]):16.8f}"
        )
    return "\n".join(lines) + "\n"
...
class ResultWriter:
    ...
    def write_exchange_outputs(
        ...
        jfile_path.write_text(uppasd_jfile_text(vectors, jij, structure))
